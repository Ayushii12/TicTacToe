...
var starting_player = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
  ...
  b.state.forEach((cell, index) => {
      html_cells[index].addEventListener('click', () => {
          ...
          let symbol = !player_turn ? 'x' : 'o'; //Maximizing player is always 'x'
...
class Player {
  ...
  getBestMove_negamax(pov, board, callback = () => {}, depth = 0) {
    ...
    if (board.isTerminal() || depth == this.max_depth) {
      ...
      board.getAvailableMoves().forEach(index => {
        ...
        let node_value =-this.getBestMove_negamax(!pov,child, callback, depth + 1);
        //Updating best value
